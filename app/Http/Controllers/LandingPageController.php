...
class LandingPageController extends Controller
{
    ...
    public function favorit()
    {
        ...
        $data['page_title'] = 'Favorit';
        $favorite = Favorit::where('pengguna_id_222058', Auth::guard('pengguna')->user()->id_222058)->get()->pluck('wisata_id_222058');
        $data['wisata_fav'] = Wisata::whereIn('id_222058',$favorite)->orderBy('created_at', 'desc')->get();

        return view('fe.homepage.favorite', $data);
    }
    ...
    public function StorepesanTiket(Request $request, $id)
    {
        ...
        try {
            ...
            // SIMPAN KE TABLE PEMBAYARAN BERDASARKAN TIKET ID 
    ...
    public function viewPembayaran($id)
    {
        ...
        // GET DATA KE TABLE TIKET BY ID TIKET 
        $data['tiket'] = Tiket::where('id_222058', $id)->first();
        // GET DATA PEMBAYARAN KE TABLE PEMBAYARAN BERDASARKAN TIKET ID  
        $data['pembayaran'] = Pembayaran::where('tiket_id_222058', $id)->first();

        // GET WISATA BY TIKET ID 
    ...
    public function processPayment(Request $request, $id)
    {
        ...
        // UPDATE BUKTI PEMBAYARAN BERDASARKAN ID PEMBAYARAN 
        $pembayaran = Pembayaran::where('id_222058', $id)->firstOrFail();

        // SIMPAN DATA JIKA FILE BUKTI PEMBAYARAN TERSEDIA 
    ...
    public function updateSudahDigunakan($id)
    {
        ...
        // UPDATE TIKET KE SUDAH DIGUNAKAN 
    ...
    public function registerStore(Request $request)
    {
        try {
            ...
            // SIMPAN DATA PENGGUNA KE TABLE PENGGUNA
            ...
            $penggunaLog = Pengguna::where('email_222058', $request->email)->first();
            if ($penggunaLog && Hash::check($request->password, $penggunaLog->password_222058)) {
                Auth::guard('pengguna')->login($penggunaLog, $request->remember);
    ...
    public function rating(Request $request, $id)
    {
        ...
        // SIMPAN DATA REVIEW BERDASARKAN ID USER YANG LOGIN DAN ID WISATA 
        $review = new Review();
    ...
    public function toggleFavorite($wisataId)
    {
        // PENGECEKAN JIKA PENGGUNA SUDAH LOGIN 
        if (Auth::guard('pengguna')->check()) {
            $user = Auth::guard('pengguna')->user();

            // Cek apakah wisata sudah ada di favorit
            $existingFavorite = Favorit::where('pengguna_id_222058', $user->id_222058)
                ->where('wisata_id_222058', $wisataId)
                ->first();

            if ($existingFavorite) {
                // Jika sudah ada, hapus dari favorit
                $existingFavorite->delete();
                return redirect()->route('wisata-detail', $wisataId)
                    ->with('success', 'Wisata dihapus dari favorit!');
            } else {
                // Jika belum ada, tambahkan ke favorit
                $favorite = new Favorit();
                $favorite->pengguna_id_222058 = $user->id_222058;
                $favorite->wisata_id_222058 = $wisataId;
                $favorite->save();

                return redirect()->route('wisata-detail', $wisataId)
                    ->with('success', 'Wisata ditambahkan ke favorit!');
            }
        }
